...
class FDCDatabase:
    ...
    def reconnect(self):
        """
        Reconnect to database (useful for recovering from connection drops).

        Closes existing connection if any, then establishes a new connection.
        """
        print("[DB] Reconnecting to database...")
        try:
            self.close()
        except Exception as e:
            print(f"[DB] Warning during close: {e}")
