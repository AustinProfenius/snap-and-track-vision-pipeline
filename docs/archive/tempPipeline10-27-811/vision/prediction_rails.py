...
class PredictionRails:
    ...
    def validate_mass_feasibility(self, food_item: Dict[str, Any]) -> Dict[str, Any]:
        """
        Check if mass is feasible for food class (mass-only mode).

        Adds rails_flag="mass_outlier" to metadata if mass is wildly out of range.
        Used for logging only, does not block alignment.

        Typical ranges:
        - Single egg: 40-70g
        - Bell pepper: 80-200g
        - Chicken breast: 100-250g
        - Rice (cooked, 1 cup): 150-200g

        Args:
            food_item: Food dict with name, mass_g

        Returns:
            Modified food_item with metadata flag if mass outlier detected
        """
        name = food_item.get("name", "").lower().strip()
        mass_g = food_item.get("mass_g", 0)
        ...
        # Define feasibility ranges (rough guidelines)
        feasibility_ranges = {
            # Discrete items
            "egg": (40, 70),
            "almond": (20, 50),  # ~20-40 nuts
            "walnut": (25, 60),
            "cashew": (25, 50),
            "strip": (80, 200),  # chicken strips, bacon strips
            "slice": (20, 100),  # bread slice, cheese slice
            ...
            # Proteins
            "chicken breast": (100, 300),
            "chicken thigh": (80, 200),
            "salmon": (100, 250),
            "steak": (150, 400),
            "bacon": (20, 80),
        ...
        # Check for matches
        for food_key, (min_g, max_g) in feasibility_ranges.items():
            if food_key in name:
                if mass_g < min_g * 0.5 or mass_g > max_g * 2.0:
                    # Mass is wildly out of range (50% below min or 2× above max)
                    print(f"[RAILS] Mass outlier: {name} has {mass_g}g (expected {min_g}-{max_g}g)")
                    if "_rails_metadata" not in food_item:
                        food_item["_rails_metadata"] = {}
                    food_item["_rails_metadata"]["mass_outlier"] = True
                    food_item["_rails_metadata"]["expected_range"] = f"{min_g}-{max_g}g"
                break
    ...
    def infer_per_unit_mass(self, food_item: Dict[str, Any]) -> Dict[str, Any]:
        """
        Infer per-unit mass for discrete items (mass-only mode).

        If count is present, compute per-unit mass and store in metadata.
        Used by fdc_alignment_v2.py for cut/size selection.

        Args:
            food_item: Food dict with name, count (optional), mass_g

        Returns:
            Modified food_item with per_unit_mass in metadata
        """
        count = food_item.get("count")
        mass_g = food_item.get("mass_g", 0)

        if not count or count <= 0 or mass_g <= 0:
            return food_item

        per_unit_mass = mass_g / count

        print(f"[RAILS] Per-unit mass: {food_item.get('name', 'unknown')} = {per_unit_mass:.1f}g per item ({count} items)")

        if "_rails_metadata" not in food_item:
            food_item["_rails_metadata"] = {}
        food_item["_rails_metadata"]["per_unit_mass"] = per_unit_mass
        food_item["_rails_metadata"]["count"] = count
    ...
    def apply_all_rails(self, prediction: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for food in prediction["foods"]:
            ...
            # NEW: Mass-only mode rails
            food = self.validate_mass_feasibility(food)
            food = self.infer_per_unit_mass(food)
